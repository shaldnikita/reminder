...
import ru.shaldnikita.users.port.adapter.dao.Tables
import ru.shaldnikita.users.port.adapter.dao.contact.{ContactDao, ContactSchema}
...
class ContactDaoSpec extends DaoSpecBase {
  ...
  val contact =
    ContactSchema(contactId, ContactType.Telegram.id, "@loopa", userId)
  ...
  "ContactDao" should {
    ...
    "find user contacts" in {
      ...
      contactDao.findUserContacts(userId).futureValue should contain allElementsOf Seq(
        contact,
        secondContact
      )
    }
