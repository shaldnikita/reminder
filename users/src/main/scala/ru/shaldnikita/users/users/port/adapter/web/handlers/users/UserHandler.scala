package ru.shaldnikita.users.users.port.adapter.web.handlers.users
...
import ru.shaldnikita.users.users.port.adapter.web.models.users.{CreateUserModel, UserModel}
import ru.shaldnikita.users.users.port.adapter.web.validators.users.{CreateUserModelValidator, UserModelValidator}
...
class UserHandler(userService: UserService)(implicit ec: ExecutionContext)
  ...
  private def deleteUser(userId: UUID) = {
    complete(
      userService
