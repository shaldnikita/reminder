package ru.shaldnikita.users.users.port.adapter.web.handlers.contacts
...
import ru.shaldnikita.users.users.port.adapter.web.handlers.BaseHandler
import ru.shaldnikita.users.users.port.adapter.web.models.contacts.{ContactModel, CreateContactModel}
import ru.shaldnikita.users.users.port.adapter.web.validators.contacts.{ContactModelValidator, CreateContactModelValidator}
...
class ContactHandler(contactService: ContactService)(
  ...
  private def getUserContacts(userId: UUID) = {
    complete(
      contactService
  ...
  private def deleteContact(contactId: UUID) = {
    complete(contactService.delete(contactId.toString).map(_ => Done))
  }

  private def deleteUserContacts(userId: UUID) = {
    complete(
